...
class AsaasGateway implements PaymentGatewayInterface
{
    public function createPayment(Payment $payment, array $options = []): array
    {
        $metadata = match ($payment->method) {
            'pix' => [
                'qr_code' => '0002010102122688...FAKE_ASAAS_PIX_QR_CODE...',
                'expires_at' => now()->addMinutes(30)->toIso8601String(),
            ],
            'credit_card' => [
                'client_secret' => 'asaas_client_secret_' . $payment->public_id,
            ],
            'boleto' => [
                'boleto_url' => 'https://example.test/asaas/boleto/' . $payment->public_id,
                'expires_at' => now()->addDays(3)->toIso8601String(),
            ],
            default => [],
        };

        $externalReference = 'as_' . $payment->public_id;
