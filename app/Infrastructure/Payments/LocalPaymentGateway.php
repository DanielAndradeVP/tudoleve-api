...
class LocalPaymentGateway implements PaymentGatewayInterface
{
    ...
    {
        // In a real implementation this would call an external provider.
        // Here we just simulate provider data and keep the payment pending.
        $metadata = match ($payment->method) {
            'pix' => [
                'qr_code' => '0002010102122688...FAKE_PIX_QR_CODE...',
                'expires_at' => now()->addMinutes(30)->toIso8601String(),
            ],
            'credit_card' => [
                'client_secret' => 'fake_client_secret_' . $payment->public_id,
            ],
            'boleto' => [
                'boleto_url' => 'https://example.test/boleto/' . $payment->public_id,
                'expires_at' => now()->addDays(3)->toIso8601String(),
            ],
            default => [],
        };

        $externalReference = 'loc_' . $payment->public_id;
    ...
    {
        // Simulate webhook handling based on an external reference.
